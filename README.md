...
`linkctl` is a Linux command-line controller for the fixed-mount Insta360 Link 2C Pro. The project is capability-driven: it will use standard Linux camera and audio interfaces first, verified device profiles second, and explicitly labelled host-side processing where appropriate.

The repository currently provides the workspace, shared contracts, and safety boundary. `linkctl --help` and `linkctl --version` are available; device and media commands are added only when their behavior can be implemented and tested honestly.
