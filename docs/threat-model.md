...
| A non-gimbal device receives mechanical commands | Omit semantic movement commands and continuously test the command graph |
...
The compiled safety policy admits read-only operations and denies raw or unknown XU writes, profile-based vendor writes, driver detach, USB reset, firmware/flash/boot writes, calibration writes, and motor operations. Configuration cannot enable a backend that is not compiled. Unsafe Rust is forbidden throughout the workspace.

There is no device I/O, daemon listener, media pipeline, network listener, native SDK loading, or profile write loader. Those boundaries require focused threat-model updates before implementation.

## Review triggers

Update this document before adding an unsafe block, device write, writable profile format, daemon IPC, arbitrary file destination, native library, model download, external process execution, network listener, firmware workflow, or diagnostic bundle containing device data.
