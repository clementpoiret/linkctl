...
| Local service | Owner-only protocol-1 IPC, camera-free idle state, serialized controls, one on-demand shared source, dormant snapshot branches, dynamic background recording/raw outputs, VA-API/software decode selection, graph/metrics, and bounded hotplug recovery |
